...
async function runTests() {
  ...
  if (failedTests > 0) {
    log('\n❌ Environment variable test failed. Please check your .env file.', 'red');
    process.exit(1);
  }
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      log(`      Sample: ${data.slice(0, 3).map(s => s.symbol).join(', ')}`, 'blue');
      passedTests++;
    }
  } catch (error) {
  ...
  try {
    ...
    log(`   ❌ Error accessing historical_prices table: ${error.message}`, 'red');
    failedTests++;
  }
  ...
  try {
    ...
    log(`   ❌ Error accessing breakout_signals table: ${error.message}`, 'red');
  ...
  try {
    ...
    const prices = [100, 102, 101, 103, 105, 104, 106, 108, 107, 109,
                    110, 111, 109, 112, 114, 113, 115, 116, 115, 117, 118];
    
    const ema = calculateEMA(prices, 20);
    
    if (ema && ema > 0) {
      log(`   ✅ EMA calculation works (20-period EMA: ${ema.toFixed(2)})`, 'green');
      passedTests++;
    } else {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      log('      Scanner needs recent historical data to work properly', 'yellow');
    } else {
      log(`   ✅ Found recent data for ${new Set(data.map(d => d.symbol)).size} symbols`, 'green');
      passedTests++;
    }
  } catch (error) {
  ...
  if (failedTests === 0) {
    ...
    log('❌ Multiple tests failed. Please fix the issues before deploying.', 'red');
